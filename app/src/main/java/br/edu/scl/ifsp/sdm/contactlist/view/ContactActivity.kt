...
class ContactActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        receivedContact?.let { received ->
            val viewContext = intent.getBooleanExtra(EXTRA_VIEW_CONTACT, false)
            with(acb){
                if (viewContext){
        ...
        with(acb){
            saveBt.setOnClickListener {
                val contact = Contact(
                    id = receivedContact?.id?:hashCode(),//pega o cod existente ou cria um novo
                ...
                resultIntent.putExtra(EXTRA_CONTACT,contact)//Chave + dados do usuário
                setResult(RESULT_OK,resultIntent)//Enviar resultado para mainActivity
